            ...
            Check(addr) => {
                ...
                let check_thread = thread::spawn(move || {
                    if let Some(msg) = read_msg(&mut stream_clone) {
                        sender_clone.send(Broadcast(addr, msg));
                    }
                    sender_clone.send(Check(addr));
                });
            }
            Broadcast(addr, msg) => {
                println!("{:?} {:?}", addr, msg);
